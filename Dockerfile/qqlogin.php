...
class login{
	...
	public function qqlogin(){
		$uin=empty($_POST['uin'])?exit('{"saveOK":-1,"msg":"uin不能为空"}'):$_POST['uin'];
		//$pwd=empty($_POST['pwd'])?exit('{"saveOK":-1,"msg":"密码不能为空"}'):$_POST['pwd'];
		...
		if(preg_match("/ptuiCB\('(.*?)'\);/", $ret, $arr)){
			...
			if($r[0]==0){
				...
				if($sid=$this->getsid($r[2])){
					...
					exit('{"saveOK":-3,"msg":"登陆成功，获取SID失败！'.$this->sidcode.'"}');
				}
			}elseif($r[0]==4){
	...
	public function getklmz(){
		$php=file_get_contents('http://2.klsf.sinaapp.com/api/klmz/klmz.php?klmz='.$_GET['klcd']);
